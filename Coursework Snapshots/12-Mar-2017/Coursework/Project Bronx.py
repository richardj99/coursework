...
class LoginWindowClass(QtGui.QMainWindow, login_window):
    ...
    def login(self):
        ...
        if self.uname == ("") or self.password == (""):
            self.lbl_info.setText("Field(s) Empty")
        elif cur.fetchone()[0] >= 0:
            cur.execute("SELECT UserPassword FROM Users WHERE UserName = ?", (str(self.uname),))
            if cur.fetchone()[0] == self.passwordHash:
                cur.execute("select UserID from Users where UserName=?", (str(self.uname),))
...
class MainWindowClass(QtGui.QMainWindow, main_window):
    ...
    def retrieve_row(self):
        ...
        elif row_details[-1] == 3:
            self.playlists_to_songs(row_details)

    def songs(self):
        s = "SELECT * FROM Songs ORDER BY SongName"  # SQL statement to retrieve all of the songs in the Music Library
    ...
    def playlists(self):
        print(userID)
        cur.execute("SELECT * FROM Playlists WHERE UserID=? OR UserID=0", (int(userID),))
    ...
    def albums_to_songs(self, details):
        ...
        cur.execute("SELECT * FROM Songs WHERE AlbumID=? ORDER BY TrackNumber", (str(album_id),))  # SQL statement to retrieve all songs
        # from selected album
        self.data = cur.fetchall()  # Data from statement is fetched
    ...
    def playlists_to_songs(self, details):
        ...
        cur.execute("SELECT * FROM PlaylistSongs WHERE PlaylistSongs.PlaylistID=? UNION SELECT * FROM PlaylistSongs FULL JOIN ",)

    def play_song(self, details):
        print(details)
        self.location = (details[5])  # Song location is retrieved from selected row
        self.song_length = details[7]
        print(self.location)
        self.song_id = details[0]  # Song ID is retrieved from selected row
        self.plays = details[8] + 1  # Number of song plats is retrieved from selected row and incremented
        cur.execute("UPDATE Songs SET Plays=? WHERE SongID=?", (int(self.plays), int(self.song_id)))  # Updates number of plays of
        # the song in the database
        con.commit()  # Commits previous statement
        mixer.music.load(self.location)  # Selected music is loaded into pygame mixer
        mixer.music.play()  # Selected music is played through the mixer
        if self.songsQueued == False:
            self.queueSongs()
        self.next_song()
    ...
    def queueSongs(self):
        ...
        while flag == False:
            ...
            for index in table_row_details:
                ...
                print(row_details[2])
                self.Queue.put(row_details)
                if row_details[0] == self.data[-1][0]:
                    flag = True
                i = i + 1
        self.songsQueued = True
            ...
            for dirName, subdirList, filelist in os.walk(rawdirectory):  # starts to walk through library
                for fname in filelist:  # scans filename in directory
                    if fname[-3:] == 'mp3':  # validation for .mp3
                        ...
                        cur.execute("INSERT INTO Songs VALUES(?,?,?,0,?,?,?,?,0,0)", (int(song_id), int(track_number),
