...
class CreateAccountWindowClass(QtGui.QMainWindow, createaccount_window):
    def __init__(self, parent=None):
        ...
        self.new_txt_username = ""
        self.new_txt_password = ""

    def create_account(self):
        self.new_username = self.txt_username.text()
        self.new_password = self.txt_password.text()
        print(self.new_username, self.new_password)
        ...
        if username_validate != 0:
            ...
            self.password_hash = hashlib.sha256(self.new_password.encode()).hexdigest()
...
class MainWindowClass(QtGui.QMainWindow, main_window):
    ...
    def settings(self):
        MainWindow.hide()  # changes current window to settings window
        SettingsWindow.show()
            ...
            for dirName, subdirList, filelist in os.walk(rawdirectory):  # starts to walk through library
                for fname in filelist:  # scans filename in directory
                    if fname[-3:] == 'mp3':  # validation for .mp3
                        ...
                        cur.execute("INSERT INTO Songs VALUES(?,?,?,0,?,?,?)", (int(song_id), str(song_name),
